...
for new_obstacle_y in range(length):
    for new_obstacle_x in range(length):
        time_paradox_tracker = []
        if original_map[new_obstacle_y][new_obstacle_x] == "#":
            print("skip")
            continue
        map = deepcopy(original_map)
        map[new_obstacle_y][new_obstacle_x] = "#"
        print(f"y: {new_obstacle_y} x: {new_obstacle_x}")
        while True:
